...
class Drawer:
    @staticmethod
    def make_table(data: dict, width: int = 3000, height: int = 2000):
        try:
            ...
            bold_font = ImageFont.truetype(io.BytesIO(open("scripts/fonts/calibri.ttf", "rb").read()), 180,
                                           encoding='UTF-8')
                ...
                if index % 2 == 0:
                    ...
                    if co1 == 2:
                        for text in range(len(data[index])):
                            if text == 0:
                                f_width, f_height = draw.textsize(str(data[index][text]) + 'Р', font=bold_font)
                                draw.text(((int(delta_width * width) + delta_width_ * width - f_width) / 2,
                                           (int(delta_height * height_) + delta_height_ - f_height) / 2),
                                          str(data[index][text]) + 'Р', fill="black", font=bold_font)
                                delta_width = 0.66
                                delta_width_ = 0.99
                            else:
                                f_width, f_height = draw.textsize(str(data[index][text]), font=bold_font)
                                draw.text(((int(delta_width * width) + delta_width_ * width - f_width) / 2,
                                           (int(delta_height * height_) + delta_height_ - f_height) / 2),
                                          str(data[index][text]), fill="black", font=bold_font)
                                delta_width = 0.66
                                delta_width_ = 0.99
                    else:
                        for text in range(len(data[index])):
